...
census_api_key("INSERT YOUR CENSUS API KEY HERE", install = TRUE, overwrite=T)
readRenviron("~/.Renviron")

# Function to get ACS data for a specific year (county level)
...
# Get ACS data for 2013-2019
years <- 2013:2019
acs <- map_dfr(years, get_county_data)

# Load CHC data
chc_df <- bind_rows(
  lapply(2013:2019, function(year) {
    read_excel(paste0("~/Downloads/UDS/UDS ", year, " Data dump.xlsx"), sheet = "HealthCenterSiteInfo") %>%
      select(SiteZIPCode) %>%
      mutate(
        site = 1,
        Year = year,
        SiteZIPCode = str_sub(SiteZIPCode, 1, 5)  # Extract first 5 characters of ZIP code
      )
  })
)
...
# limit the data to years 2013 through 2019
...
prepare_data <- function(data) {
  ...
  # Calculate CHC change between 2013 and 2015
  chc_change <- data %>%
    ...
    summarise(
      ...
      sites_2015 = mean(as.numeric(sites[year == 2015]), na.rm = TRUE),
      .groups = "drop"
    ) %>%
    mutate(
      chc_change_2013_2015 = sites_2015 - sites_2013,
      chc_loss = as.integer(chc_change_2013_2015 < 0)
    )
  ...
  prepared_data <- data %>%
    ...
    mutate(
      across(c(education_percent, median_income, children, elderly, female, 
             ...
             list(
               baseline = ~first(as.numeric(.x[year == 2013])),
               change = ~last(as.numeric(.x[year == 2019])) - first(as.numeric(.x[year == 2013]))
             )),
      baseline_sites = first(as.numeric(sites[year == 2013])),
      baseline_mortality = first(as.numeric(val[year == 2013])),
...
create_chc_change_map <- function(data) {
  ...
  ggplot(map_data, aes(long, lat, group = group, fill = change_sites)) +
    ...
    scale_fill_viridis(option = "magma", name = "Change in CHC Sites",
                       ...
                       oob = scales::squish) +
    labs(title = "Change in CHC Sites by County (2013-2015)",
...
analyze_matched_data <- function(data, match_obj, outcome_var) {
  ...
  # Regression model with Primary care physicians as mediator
  reg_model_pcp <- lm(paste(outcome_var, "~ chc_loss * factor(year) + baseline_mortality + baseline_sites + `Primary care physicians`"), 
                      data = full_matched_data, weights = weights)
  robust_se_pcp <- coeftest(reg_model_pcp, vcov = vcovHC(reg_model_pcp, type = "HC3"))
  
  return(list(
    original = tidy(robust_se),
    with_pcp = tidy(robust_se_pcp)
  ))
}

# Debugging function
print_result_structure <- function(result) {
  cat("Structure of result:\n")
  print(str(result))
  
  if (!is.null(result$analysis_result)) {
    cat("\nStructure of analysis_result:\n")
    print(str(result$analysis_result))
  } else {
    cat("\nanalysis_result is NULL\n")
  }
}

# Modified compare_results function
compare_results <- function(result) {
  print_result_structure(result)  # Debug print
  
  if (is.null(result) || is.null(result$analysis_result)) {
    cat("Analysis result is NULL or doesn't contain analysis_result.\n")
    return(NULL)
  }
  
  if (!("original" %in% names(result$analysis_result)) || 
      !("with_pcp" %in% names(result$analysis_result))) {
    cat("analysis_result doesn't contain 'original' or 'with_pcp' components.\n")
    return(NULL)
  }
  
  original <- result$analysis_result$original %>% 
    filter(term == "chc_loss") %>% 
    mutate(model = "Without PCP")
  
  with_pcp <- result$analysis_result$with_pcp %>% 
    filter(term == "chc_loss") %>% 
    mutate(model = "With PCP")
  
  combined_results <- bind_rows(original, with_pcp) %>%
...
# For overall mortality (total, all races)
overall_result <- tryCatch({
  chc_impact_analysis(
    prepared_data %>% filter(mortality_type == "Total" & race_name == "Total"), 
    covariates, 
    "val"
  )
}, error = function(e) {
...
sensitivity_results <- compare_results(overall_result)

if (!is.null(sensitivity_results)) {
  print(sensitivity_results)
  
  # Create a formatted table for the sensitivity analysis results
  library(gt)
  
  sensitivity_table <- sensitivity_results %>%
    gt() %>%
    fmt_number(
      columns = c(estimate, std.error, ci_lower, ci_upper),
      decimals = 3
    ) %>%
    fmt_number(
      columns = p.value,
    ...
    tab_header(
      title = "Sensitivity Analysis: Impact of Including Primary Care Physicians",
      subtitle = "Effect of CHC Loss on Total Mortality (All Races)"
    )
  
  # Save the sensitivity analysis table
  gtsave(sensitivity_table, "sensitivity_analysis_pcp.html")
  cat("\nSensitivity analysis results saved as 'sensitivity_analysis_pcp.html'.\n")
  
  # Calculate the percentage change in the effect estimate
  percent_change <- (sensitivity_results$estimate[2] - sensitivity_results$estimate[1]) / 
    sensitivity_results$estimate[1] * 100
  
  cat("\nPercentage change in effect estimate after including Primary Care Physicians:", 
      round(percent_change, 2), "%\n")
} else {
  cat("Unable to perform sensitivity analysis due to invalid results.\n")
}